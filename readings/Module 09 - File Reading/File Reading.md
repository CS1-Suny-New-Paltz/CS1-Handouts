...
Text is surprisingly flexible file format - in theory, any data (including imagery data) could be represented as a text file. In practice, text is not the most **compact** way to store data, which is why there are different, more specialized, formats for large data types like images and videos. Text formatting is particularly nice because it's **human-readable** - you can open a file in a text editor and make some sense out of it. Fun experiment: open the .classpath file in your last assignment directory in a text editor (you may have to switch Finder/File Explorer to show hidden files to do this). While the file may not make a lot of sense, since we haven't gone over the xml schema for Eclipse classpaths, you'll notice that it looks a lot more readable than opening, for example, the assignDeps.jar file in a text editor.

The .classpath file highlights another common feature of text files - they usually have an additional layer of formatting that's **implied** and not built in to the structure of the bits for the file. If you were to open a .pdf file in a text editor and delete some of the gibberish that shows up, you probably wouldn't be able to open the file as a pdf afterwards - the data has been corrupted. If you were to delete a line out of the .classpath file, though, you would still be able to read it as a text file. However, depending on what line you removed, Eclipse might have trouble understanding the .classpath afterwards - it's expecting not just a text file, but a text file with a **specific layout**. You may have run into something similar in some of your classes - if you're required to cite sources in MLA format for a research paper, you have to format the text in your file in a very specific way.

## Reading a File

Reading a file comes in two parts: moving the underlying bits from the hard drive into computer memory where a program can interact with them (this is also often called **loading** a file), and **parsing** the bits into something useful. For very common file formats, those two steps can be combined using existing **libraries** of code. For now, we’re going to focus on reading files that use a text format for their data \- it's easier to debug file reading when you can open the file in a text editor and see what's going on. Once you've mastered reading text files, reading more compact custom formats isn't much more complicated - you'll just be interacting with bytes instead of characters.
